...
def phonetic_scrape(word: str):
    ...
    try:
        ...
        word_scraped = str(list(soup.findAll('h1', {'class': "css-urzbm0 e1rg2mtf5"}))[0]).lower()
        word_scraped = word_scraped[word_scraped.find(">") + 1: word_scraped[1:].find("<") + 1]
        append_where = 0

        if word_scraped != word:

            (to_append, append_where) = helper.__phonetic_scrape_helper(word, word_scraped)
            if to_append == -1:
                print("Need to fix!")
                raise IndexError
        ...
        for i in range(len(raw_phonetic)):
            ...
            if (raw_phonetic[i] == ">"):
                ...
                raw_phonetic, next_stop = helper.__update_next_stop(raw_phonetic, next_stop, i)
                ...
                if next_stop > i + 1 and syllable != "]" and syllable != "[" and syllable != "-":
                    ...
                    phonetic[counter_phonetic] = (phonetic[counter_phonetic]).encode('ascii', 'ignore').decode(
                        'unicode_escape')
